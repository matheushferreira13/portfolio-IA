...
async function loadKnowledge() {
  ...
  const raw = await readFile(path.join(__dirname, 'assets', 'data', 'portfolio-knowledge.json'), 'utf8');
...
async function callGemini({ question, history, context, factualGuide }) {
  if (!GEMINI_API_KEY) {
...
async function completeIfNeeded({ question, partial, history, context, factualGuide, knowledge }) {
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const context = retrieveContext(question, docs, 5);
    const factualGuide = [recruiterGuide(question, knowledge), portfolioGuide(question, knowledge)].filter(Boolean).join('\n');

    let answer = await callGemini({ question, history, context, factualGuide });
    answer = await completeIfNeeded({ question, partial: answer, history, context, factualGuide, knowledge });
    ...
    const docs = knowledge ? buildKnowledgeDocs(knowledge) : [];
    const context = retrieveContext(req.body?.question || '', docs, 5);

    if (detail.includes('429')) {
      return res.status(429).json({
        error: 'Limite temporario da API atingido.',
        fallback: buildFallback(req.body?.question || '', context)
      });
    }

    console.error('Erro no backend Gemini:', error);
    return res.status(500).json({
      error: 'Erro ao conectar com a IA no backend.',
      fallback: buildFallback(req.body?.question || '', context)
    });
  }
});
...
app.listen(PORT, () => {
  console.log(`Servidor do portfolio ativo em http://localhost:${PORT}`);
});
